...
public class MetaballRenderPass : ScriptableRenderPass
{
    ...
    {
        RenderTextureDescriptor descriptor = new RenderTextureDescriptor(Screen.width, Screen.height, RenderTextureFormat.ARGB32, 24);
        descriptor.depthBufferBits = 24;
        RenderTexture tempRT = RenderTexture.GetTemporary(descriptor);

        cmd.Blit(src, tempRT);
        cmd.SetRenderTarget(tempRT, RenderBufferLoadAction.Load, RenderBufferStoreAction.Store, tempRT.depthBuffer, RenderBufferLoadAction.Load, RenderBufferStoreAction.Store);
